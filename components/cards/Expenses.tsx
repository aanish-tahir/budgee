...
export default function ExpensesCard() {
  ...
  return (
    <Box
          ...
          paddingY={2}
        >
        <Text color="gray.700" fontSize="md">Your Monthly Expense</Text>
...
function ExpenseItem({category, spent}:{category:string, spent:number}){
    return (
        <Box>
            <Text color="gray.700" fontSize="xs">{category}</Text>
            <Text color="black" fontSize="xs" fontWeight="medium">{currencyFormat(spent)}</Text>
        </Box>
    )
}
